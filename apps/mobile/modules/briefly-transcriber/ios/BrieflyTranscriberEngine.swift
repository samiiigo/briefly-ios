// BrieflyTranscriberEngine.swift
...
struct BrieflyTranscriberEventSink {
  var isListening: () -> Bool = { true }
  var emit: (_ name: String, _ body: [String: Any]) -> Void = { _, _ in }
}

enum BrieflyTranscriberError: Error {
  case code(String, String)

  var nsError: NSError {
    switch self {
    case .code(let code, let message):
      return NSError(domain: "BrieflyTranscriber", code: 1, userInfo: [
...
final class BrieflyTranscriberEngine {
  private let eventSink: BrieflyTranscriberEventSink

  init(eventSink: BrieflyTranscriberEventSink) {
    self.eventSink = eventSink
  }

  private var hasListeners: Bool { eventSink.isListening() }
  ...
  private func sendEvent(_ name: String, body: [String: Any]) {
    eventSink.emit(name, body)
  }

  // MARK: - Live Transcription: Start

  func startLiveTranscription(options: NSDictionary) throws -> [String: Any] {
    ...
    DispatchQueue.main.async {
      do {
        result = try self.doStartLive(options: options)
      } catch {
        thrown = error
      }
      group.leave()
  ...
  private func doStartLive(options: NSDictionary) throws -> [String: Any] {
    if isLive {
      throw BrieflyTranscriberError.code("ALREADY_LIVE", "Live transcription session already in progress.")
    }
    ...
    guard let apiKey = resolveAssemblyAIApiKey(suppliedApiKey: suppliedApiKey) else {
      throw BrieflyTranscriberError.code(
        "MISSING_API_KEY",
    ...
    client.onPartial = { [weak self] text in
      ...
      self.sendEvent("onPartialTranscript", body: ["text": text])
    }
    client.onFinal = { [weak self] text in
      guard let self, self.hasListeners else { return }
      self.sendEvent("onFinalTranscript", body: ["text": text])
    }
    ...
    client.onState = { [weak self] state, reason in
      guard let self, self.hasListeners else { return }
      self.sendEvent("onStreamingState", body: [
    ...
    do {
      ...
      throw BrieflyTranscriberError.code("AUDIO_CAPTURE", error.localizedDescription)
    }
    ...
    isPaused = false
    liveStartDate = Date()
    return ["sampleRate": sampleRate, "speechModel": speechModel]
  }

  // MARK: - Live Transcription: Pause

  func pauseLiveTranscription() throws {
    guard isLive else { return }
    isPaused = true
    audioCapture?.pause()
    if hasListeners {
      sendEvent("onStreamingState", body: ["state": "paused"])
    }
  }

  // MARK: - Live Transcription: Resume

  func resumeLiveTranscription() throws {
    guard isLive else {
      throw BrieflyTranscriberError.code("NOT_LIVE", "No active live transcription session")
    }
    do {
      try audioCapture?.resume()
      isPaused = false
      if hasListeners {
        sendEvent("onStreamingState", body: ["state": "open"])
      }
    } catch {
      throw BrieflyTranscriberError.code("ENGINE_START", error.localizedDescription)
    }
  }

  // MARK: - Live Transcription: Stop

  func stopLiveTranscription() throws -> [String: Any] {
    ...
    let payload: [String: Any] = ["uri": uri, "duration": duration]
    let group = DispatchGroup()
    group.enter()
  ...
  func startOnDeviceLiveTranscription(options: NSDictionary) throws -> [String: Any] {
    _ = options
    if isLive {
      throw BrieflyTranscriberError.code("ALREADY_LIVE", "Live transcription session already in progress.")
    }
    ...
    do {
      ...
      isPaused = false
      liveStartDate = Date()
      return ["sampleRate": 0, "speechModel": "on-device"]
    } catch {
      throw BrieflyTranscriberError.code("ONDEVICE_START", error.localizedDescription)
    }
  }

  func pauseOnDeviceLiveTranscription() throws {
    guard isLive, let session = onDeviceSession else { return }
    isPaused = true
    session.pause()
  }

  func resumeOnDeviceLiveTranscription() throws {
    guard isLive, let session = onDeviceSession else {
      throw BrieflyTranscriberError.code("NOT_LIVE", "No active on-device live transcription session")
    }
    do {
      try session.resume()
      isPaused = false
  ...
  func stopOnDeviceLiveTranscription() throws -> [String: Any] {
  ...
  func startAudioCapture(options: NSDictionary) throws -> [String: Any] {
    var result: [String: Any]?
    var thrown: Error?
    let group = DispatchGroup()
    group.enter()
    DispatchQueue.main.async {
      do {
        if self.isCaptureActive {
          throw BrieflyTranscriberError.code("ALREADY_CAPTURING", "Audio capture already in progress.")
        }
        ...
        capture.onPCMChunk = { [weak self] data in
          ...
          self.sendEvent("onPCMChunk", body: ["data": b64])
        }
        capture.onError = { [weak self] message in
          self?.sendError(message)
        }
      ...
      } catch let error as BrieflyTranscriberError {
        thrown = error
      } catch {
        thrown = BrieflyTranscriberError.code("AUDIO_CAPTURE", error.localizedDescription)
      }
      group.leave()
    }
    group.wait()
  ...
  func pauseAudioCapture() throws {
    captureSes?.pause()
  }

  func resumeAudioCapture() throws {
    do {
  ...
  func stopAudioCapture() throws -> [String: Any] {
    ...
    return ["uri": uri, "duration": duration]
  }
  ...
  private func sendError(_ message: String) {
    ...
    sendEvent("onTranscriptionError", body: ["message": message])
  }
