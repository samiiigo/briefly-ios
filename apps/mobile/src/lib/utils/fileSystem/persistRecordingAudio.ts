...
import { extensionFromFilename } from '@/features/recording/utils/importKind';
import { recordingAudioDestName } from '@/features/recording/utils/recordingAudioFilename';
...
function destFileCandidates(recordingId: string, sourcePath: string): File[] {
  ...
  const extensions = [hinted, ...RECORDING_AUDIO_EXTENSIONS.filter((ext) => ext !== hinted)].filter(
    Boolean,
  ) as string[];
  return extensions.map((ext) => new File(Paths.document, `rec-${recordingId}${ext}`));
}
...
export function repairRecordingFilePaths(recordings: Recording[]): {
  ...
  const repaired = recordings.map((recording) => {
    ...
    if (stored === resolved.filePath && recording.fileSize === resolved.fileSize) {
      return recording;
    }
    changed = true;
    return {
