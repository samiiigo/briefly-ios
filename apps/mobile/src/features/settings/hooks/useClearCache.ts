...
import { clearAppCache } from '@/lib/utils/fileSystem/clearAppCache';
import { formatFileSize } from '@briefly/utils';
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            try {
              ...
              const sizeLabel = freedBytes > 0 ? ` (${formatFileSize(freedBytes)})` : '';
              Alert.alert(
                'Cache cleared',
                `Removed ${deletedCount} item${deletedCount === 1 ? '' : 's'}${sizeLabel}.`,
              );
            } catch (error: unknown) {
              const message = error instanceof Error ? error.message : 'Could not clear the cache.';
