...
import { createScopedJsonStorage } from '@/core/services/storage/storageScope';
...
import { registerLocalLlmDownloadStateSetter } from '@/features/processing/summarization/local/localLlmDownloadState';
import {
  loadProviderApiKeysFromSecureStore,
  saveProviderApiKey,
} from '@/core/security/secureApiKeyStore';
import { ValidationError } from '@briefly/validation';
/**
  ...
  persist(
    (set, get) => ({
      ...
      deleteLocalLlmModel: async () => {
        const { deleteLocalGemmaModel } =
          await import('@/features/processing/summarization/local/gemmaModelDownload');
