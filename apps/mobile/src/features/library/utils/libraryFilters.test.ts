...
describe('library filters', () => {
  ...
  it('filters archived and favorites correctly', () => {
    assert.deepEqual(
      filterByLibraryTab(recordings, 'favorites').map((r) => r.id),
      ['2'],
    );
    assert.deepEqual(
      filterByLibraryTab(recordings, 'archived').map((r) => r.id),
      ['3'],
    );
  });
  it('excludes archived recordings from errors tab (archived is a separate bucket)', () => {
    ...
    assert.deepEqual(
      filterByLibraryTab(withArchivedError, 'errors').map((r) => r.id),
      ['5'],
    );
    assert.deepEqual(
      filterByLibraryTab(withArchivedError, 'archived').map((r) => r.id),
      ['3', '6'],
    );
  });
  it('excludes recently-deleted recordings from all library tabs', () => {
    const withDeleted = [...recordings, mkRecording('6', { deletedAt: Date.now() })];
    assert.equal(filterByLibraryTab(withDeleted, 'all').length, 5);
    assert.deepEqual(
      filterByLibraryTab(withDeleted, 'active').map((r) => r.id),
      ['1', '4', '5'],
    );
  });
});
