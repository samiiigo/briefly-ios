...
import { formatRecentsCardDate } from '@/lib/utils';
import type { RecordingListGroupPosition } from '@briefly/utils';
import { RecordingAvatar } from '@/features/recording/components/RecordingAvatar';
...
export function RecentsEntryCard({ recording, groupPosition = 'only' }: Props) {
  ...
  return (
    <View style={groupedCardStyle(groupPosition, colors)}>
      {showDivider ? (
        <View style={[styles.rowDivider, processing ? styles.rowDividerProcessing : null]} />
      ) : null}
