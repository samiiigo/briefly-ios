...
} from '@/features/library/state/useLibraryFolderPreferencesStore';
    ...
    <View style={sh.optionGroup}>
      {options.map((opt, i) => {
        ...
        return (
          <TouchableOpacity
            key={opt.id}
            style={[sh.optionRow, !isLast && sh.optionRowBorder, selected && sh.optionRowSelected]}
            onPress={() => onSelect(opt.id)}
            activeOpacity={0.7}
          >
            <Text style={[sh.optionLabel, selected && sh.optionLabelSelected]}>{opt.label}</Text>
...
export function FolderDateFilterSection({ spaced }: { spaced?: boolean }) {
  ...
  return (
    <>
      ...
      <SheetOptionGroup options={DATE_OPTIONS} selectedId={datePreset} onSelect={setDatePreset} />
    </>
  );
}
