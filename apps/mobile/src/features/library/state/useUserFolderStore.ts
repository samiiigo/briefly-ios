...
import { UserFolder } from '@briefly/types';
import { FolderStorageService, type FolderRepository } from '@/core/services/storage';
import { generateId } from '@/lib/utils';
import { MAX_PINNED_FOLDERS } from '@briefly/config';
import { sortUserFolders } from '@/features/library/utils/userFolderSort';
import { validateFolderName, validateUserFolderId, ValidationError } from '@briefly/validation';
...
export const useUserFolderStore = create<UserFolderStore>((set, get) => ({
  ...
  toggleFolderPinned: async (id: string) => {
    ...
    await folderRepository.save(updated);
    set((state) => ({
      folders: sortUserFolders(state.folders.map((f) => (f.id === id ? updated : f))),
    }));
  },
