...
} from '@/features/library/state/useFolderBrowsePreferencesStore';
import { buildFolderSections } from '@/features/library/utils/folderBrowse';
import {
  applyFavoritesOnlyFilter,
  filterRecordingsForFolder,
} from '@/features/library/utils/filterFolderRecordings';
import { useRecordingRename } from '@/features/recording/hooks/useRecordingRename';
import { useSwipeableListChrome } from '@/lib/hooks/common/useSwipeableListChrome';
import type { Recording } from '@briefly/types';
import type { RecordingListGroupPosition } from '@briefly/utils';
...
export function useFolderRecordingsScreen(params: FolderRecordingsRouteParams) {
  ...
  const openRecording = useCallback((id: string) => router.push(`/recording/${id}`), [router]);
