...
export class ProcessingFailure extends Error {
  ...
  constructor(phase: ProcessingFailurePhase, message: string, summarizationMode?: ProcessingMode) {
    super(message);
...
export function isAudioFileMissingError(error: unknown): boolean {
  const message = (error instanceof Error ? error.message : String(error)).toLowerCase();
  return message.includes('audio file not found') || message.includes('no audio file was saved');
}
