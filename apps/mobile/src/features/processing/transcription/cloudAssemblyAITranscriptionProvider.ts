 ...
 */
import { TranscriptSegment } from '@briefly/types';
import { getPathInfo } from '@/lib/utils/fileSystem/pathInfo';
import { proxyAssemblyAIClient } from './proxyAssemblyAIClient';
import { MIN_TRANSCRIPTION_AUDIO_BYTES } from './transcriptionAudioConstants';
import { logger } from '@/lib/utils/logging/logger';
import type { AssemblyAITranscriptPayload } from './assemblyAIClient';
...
export class CloudAssemblyAITranscriptionProvider implements TranscriptionProvider {
  ...
  async transcribe(
    ...
    const uploadPath = await this.deps.client.uploadAudio(audioUri, apiKey);
    const transcriptId = await this.deps.client.createTranscriptJob(uploadPath, apiKey);
