/**
 * LLM JSON summary parsing
 *
 * Structured-schema mapping and extractive fallback — independent from HTTP fetch
 * helpers in summarizationUtils.
 */
import { KeyInsight } from '@briefly/types';
import { logger } from '@/lib/utils/logging/logger';
import { normalizeMainEmoji } from '@/features/recording/utils/recordingContentEmoji';
import { SummarizationResult } from './summarizationProvider';
import { generateId } from './summarizationIds';
...
export function structuredSummaryToResult(parsed: StructuredSummaryJson): {
  summary: string;
...
export function extractiveSummarize(text: string): { summary: string; keyInsights: KeyInsight[] } {
  const sentences = text
    .split(/[.!?]+/)
    .map((s) => s.trim())
    .filter((s) => s.length > 20);
  const overview = sentences.slice(0, 3).join('. ') + (sentences.length > 3 ? '.' : '');
...
export function parseJsonSummary(raw: string, fallbackText: string): SummarizationResult {
  const cleaned = raw
    .replace(/^```(?:json)?\s*/i, '')
    .replace(/\s*```$/, '')
    .trim();
  ...
  return normalizeSummarizationResult(
    {
      ...
      mainEmoji: normalizeMainEmoji(parsed.mainEmoji as string | undefined),
