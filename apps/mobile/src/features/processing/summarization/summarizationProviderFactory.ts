import { ProcessingMode, CloudProvider } from '@briefly/types';
import { ProxySummarizationProvider } from './proxySummarizationProvider';
...
class UserKeyCloudProviderResolver {
  ...
  resolve(): SummarizationProvider {
    ...
    if (!apiKey) {
      ...
      throw new Error(
        `${providerLabel} API key is not configured. Go to Settings to add your API key.`,
      );
    }
  ...
  return new ModeBasedSummarizationProviderFactory(
    {
      ...
      'cloud-shared-openrouter': () => new ProxySummarizationProvider(),
