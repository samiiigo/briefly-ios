import { MAX_RECENT_SEARCHES } from '@briefly/config';
import { validateSearchQuery, ValidationError } from '@briefly/validation';
...
export function sanitizeRecentSearchList(
  ...
  const pruned = deduped.filter((entry, index) => {
    return !deduped.some(
      (other, otherIndex) => otherIndex !== index && isStrictPrefixOf(entry, other),
    );
  });
