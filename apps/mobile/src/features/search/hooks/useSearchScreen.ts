...
import { useDebouncedValue } from '@/lib/hooks/common/useDebouncedValue';
import { SEARCH_DEBOUNCE_MS } from '@briefly/config';
import { buildSearchCatalog, runIndexedSearch } from '@/features/search/utils/searchIndex';
import { normalizeSearchQuery } from '@/features/search/utils/searchEngine';
import { filterRecentQueriesWithHits } from '@/features/search/utils/searchPristineContent';
import { isRecordingEntryNavigationLocked } from '@/features/recording/utils/recordingEntryAccess';
export function useSearchScreen() {
  ...
  const results = useMemo(() => runIndexedSearch(deferredQuery, catalog), [deferredQuery, catalog]);
