...
import { pickImportDocument } from '@/features/recording/services/pickImportDocument';
...
import { normalizeFileUri } from '@/lib/utils/fileSystem/normalizeFileUri';
import {
  detectImportKind,
...
} from '@/features/recording/utils/transcriptBackup';
...
} from '@/features/recording/utils/importDeduplication';
...
async function importJsonBackup(jsonText: string): Promise<{ count: number; skipped: number }> {
  ...
  const skippedNote =
    skipped > 0 ? ` ${skipped} duplicate${skipped === 1 ? '' : 's'} will be skipped.` : '';
...
export async function importFromPicker(): Promise<ImportRecordingResult | null> {
  ...
  if (kind === 'json-backup') {
    ...
    const skippedNote =
      skipped > 0 ? ` ${skipped} duplicate${skipped === 1 ? '' : 's'} were skipped.` : '';
    Alert.alert(
      'Import complete',
