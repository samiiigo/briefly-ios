...
export class StoreBackedRecordingBackgroundStore implements RecordingBackgroundStorePort {
  ...
  updateRecording(id: string, updates: Partial<import('@briefly/types').Recording>) {
    return useRecordingStore.getState().updateRecording(id, updates);
  }
}
export class StoreBackedBackgroundProcessingSettings implements BackgroundProcessingSettingsPort {
...
export function configureBackgroundProcessingPorts(next: Partial<BackgroundProcessingPorts>): void {
