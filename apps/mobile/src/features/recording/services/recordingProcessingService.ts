import { ProcessingMode, TranscriptSegment, TranscriptionMode } from '@briefly/types';
...
import { ensureUploadableAudioUri } from '@/lib/utils/fileSystem/repairWavForUpload';
...
} from '@/features/processing/utils/processingErrors';
import {
...
export async function retranscribeRecordingFromAudio(
  ...
  const segments = await TranscriptionService.transcribe(uploadUri, undefined, 'cloud');
  assertTranscriptHasContent(segments);
  return segments;
}
/**
...
export async function processRecordingToReady(
  ...
  if (!filePath?.trim()) {
    throw new ProcessingFailure('transcription', 'No audio file was saved for this recording.');
  }
  const pipeline = resolvePostRecordingPipeline(settingsTranscriptionMode, existingTranscript);
