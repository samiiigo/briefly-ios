...
} from '@root/modules/briefly-transcriber';
import { supportsOnDeviceLiveTranscription } from '@/lib/utils/platformCapabilities';
import { AssemblyAIConfig } from '@briefly/config/api/assemblyAI';
import { getAssemblyAISharedApiKey } from '@/core/security/sharedApiKeys';
export type AssemblyAIConnectionState =
  'idle' | 'connecting' | 'open' | 'reconnecting' | 'paused' | 'closed';
...
export class AssemblyAILiveTranscriptionClient {
  ...
  static get isSupported(): boolean {
    return (
      (Platform.OS === 'ios' || Platform.OS === 'android') && getBrieflyTranscriberModule() != null
    );
  }
  ...
  private attachListeners(): void {
    ...
    this.subscriptions.push(
      addBrieflyTranscriberListener(
        'onStreamingState',
        (e: { state: string; reason?: string | null }) => {
