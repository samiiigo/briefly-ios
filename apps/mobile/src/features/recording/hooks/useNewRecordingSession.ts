...
import { useTimer } from '@/lib/hooks/common/useTimer';
import { useLiveTranscript } from '@/features/recording/hooks/useLiveTranscript';
import { useAppInterruptGuard } from '@/lib/hooks/common/useAppInterruptGuard';
...
import {
  ...
  type NewRecordingSaveParams,
} from '@/features/recording/hooks/useRecordingStopSave';
import {
...
export function useNewRecordingSession(options: UseNewRecordingSessionOptions = {}) {
  ...
  const { isStopping, handleStop, handleDiscard, executeStopAndSaveRef } = useRecordingStopSave({
    capture,
    saveParams,
    elapsed,
    elapsedRef,
