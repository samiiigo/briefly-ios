...
} from '@/lib/utils/summary/parseSummaryMarkdown';
...
function formatKeyInsightsForExport(recording: Recording, format: 'html' | 'plain'): string {
...
function formatSummaryForExport(recording: Recording, format: 'html' | 'plain'): string {
  const summaryRaw = recording.summary?.trim() ?? '';
  if (!summaryRaw) {
    return format === 'html'
