...
describe('recordingEntryAccess', () => {
  it('locks navigation while transcribing or summarizing', () => {
    assert.equal(isRecordingEntryNavigationLocked(baseRecording({ status: 'transcribing' })), true);
    assert.equal(isRecordingEntryNavigationLocked(baseRecording({ status: 'summarizing' })), true);
  });
