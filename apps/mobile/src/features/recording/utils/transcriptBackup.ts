...
import { generateId, ensureUniqueTitle } from '@/features/recording/utils/recording';
...
function parseTranscriptSegments(raw: unknown): TranscriptSegment[] | undefined {
  ...
  for (const item of raw) {
    ...
    segments.push({
      ...
      ...(asString(item.speakerInitial) ? { speakerInitial: asString(item.speakerInitial) } : {}),
    });
  }
...
export function backupEntriesToRecordings(
  ...
  for (const entry of entries) {
    ...
    titles.push(title);
    const hasContent = hasMeaningfulTranscript(entry.transcript) || !!entry.summary?.trim();
    imported.push({
      id: generateId(),
      title,
