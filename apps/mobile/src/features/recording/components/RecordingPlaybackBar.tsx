...
import { BottomChromeOverlay } from '@/navigation/chrome/BottomChromeOverlay';
import {
  useCreateStyles,
...
export function RecordingPlaybackBar({ recording, playback, paddingBottom }: Props) {
  ...
  const { isPlaying, playbackDur, trackWidth, animatedProgress, togglePlayPause, seekToRatio } =
    playback;
  ...
  const durationLabelView = <Text style={styles.durationText}>{durationLabel}</Text>;
  return (
