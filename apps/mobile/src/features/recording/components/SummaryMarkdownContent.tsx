...
import { useCreateStyles, withAppFont } from '@briefly/theme/native';
import type { ColorPalette } from '@briefly/theme';
import { SummaryMarkdownBlock } from '@/lib/utils/summary/parseSummaryMarkdown';
...
function renderInline(text: string, baseStyle: TextStyle, styles: SummaryMarkdownStyles) {
  return (
        ...
        <View key={index} style={styles.list}>
          {block.items.map((item, itemIndex) => (
            <View key={`${index}-${itemIndex}`} style={styles.listItem}>
              ...
              <View style={styles.listItemText}>{renderInline(item, styles.body, styles)}</View>
            </View>
          ))}
        </View>
      );
