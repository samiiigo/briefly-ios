import React, { useCallback, useEffect, useReducer, useRef, isValidElement } from 'react';
import {
  View,
  StyleSheet,
  Pressable,
...
} from '@/features/recording/utils/recordingEntryAccess';
...
import { BUILTIN_MOVE_ORDER, BUILT_IN_FOLDERS } from '@briefly/config';
...
import { TextInputDialog } from '@briefly/ui/native';
import { AnchoredMenuModal, useAnchoredMenu, type AnchoredMenuItem } from '@briefly/ui/native';
...
export function RecordingSwipeableRow({
  ...
  const moveDestinations = useCallback(
    (folderList = folders): MoveDestination[] => {
  ...
  const moveMenuItems = React.useMemo((): AnchoredMenuItem[] => {
    return moveDestinations().map((dest) => ({
      label:
        dest.type === 'built-in' ? BUILT_IN_FOLDERS.find((f) => f.id === dest.id)!.name : dest.name,
