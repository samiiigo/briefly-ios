...
import { Spacing, useCreateStyles, useThemedColors } from '@briefly/theme/native';
...
export function RecordingAvatar({
  ...
  return (
    <View
      style={[styles.shell, compact ? styles.shellCompact : trailingSpacing && styles.shellList]}
    >
      {showFlash ? (
        ...
        <View style={[styles.stateCircle, compact && styles.stateCircleCompact]}>
          <Ionicons name="alert" size={compact ? 26 : 28} color={colors.orange} />
        </View>
      ) : (
        <RecordingEmojiCircle emoji={getRecordingContentEmoji(recording)} size="md" />
      )}
    </View>
  );
}
