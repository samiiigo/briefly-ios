...
import { RecordingStorageService, type RecordingRepository } from '@/core/services/storage';
import { folderFlagsFor } from '@/features/library/utils/recordingFolder';
import { logger } from '@/lib/utils/logging/logger';
import { repairRecordingFilePaths } from '@/lib/utils/fileSystem/persistRecordingAudio';
import {
  validateRecordingId,
...
function purgeExpiredRecentlyDeleted(
  ...
  const kept = recordings.filter(
    (recording) => !recording.deletedAt || recording.deletedAt > cutoff,
  );
...
function mergeRecordingsFromPersistence(memory: Recording[], persisted: Recording[]): Recording[] {
...
export const useRecordingStore = create<RecordingStore>((set, get) => ({
  ...
  updateRecording: async (id, updates) => {
    ...
    set((state) => ({
      recordings: state.recordings.map((r) => (r.id === safeId ? { ...r, ...safeUpdates } : r)),
    }));
  ...
  deleteRecording: async (id) => {
    ...
    set((state) => ({
      recordings: state.recordings.map((r) => (r.id === id ? { ...r, ...flags } : r)),
    }));
  },
