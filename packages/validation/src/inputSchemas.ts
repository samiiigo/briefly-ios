import type { CloudProvider } from '@briefly/types';
import { isValidApiKeyFormat } from '@briefly/utils';
...
export function validateRecordingTitle(title: string): string {
  const { title: validated } = validateObject<{ title: string }>({ title }, recordingTitleSchema);
  return validated;
}
