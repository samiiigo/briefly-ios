...
async function sendToSentry(dsn: string, error: unknown, context?: Record<string, unknown>): Promise<void> {
  ...
  const payload = {
    ...
    tags: {
      app: 'briefly',
      variant: String(
        (Constants.expoConfig?.extra as Record<string, unknown> | undefined)?.appVariant ?? 'unknown',
      ),
    },
  };
