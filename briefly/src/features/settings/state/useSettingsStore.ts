...
import { createScopedJsonStorage } from '@/shared/services/storage/storageScope';
...
import {
  ...
  saveProviderApiKey,
} from '@/shared/security/secureApiKeyStore';
    ...
    (set, get) => ({
      ...
      deleteLocalLlmModel: async () => {
        const { deleteLocalGemmaModel } = await import(
          '@/features/processing/summarization/local/gemmaModelDownload'
        );
